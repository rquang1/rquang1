...
def convert_ttl_to_jsonld(ttl_file, sas_token, jsonld_dir, blob_path) :
    """
    ...
    try:
        ...
        jsonld_file_path = os.path.join(jsonld_dir, jsonld_filename)
        ...
        blob_client = blob_service_client.get_blob_client(container=container_name, blob=os.path.join(blob_path, os.path.basename(jsonld_filename)))
...
def session_options_definition():
    return [
        ...
        },
        {
            'content_type': 'text',
            'id': 'blob_token',
            'name': 'Token for Blob storage',
            'description': 'Token for the Azure blob storage.'
        },
        {
            'content_type': 'text',
            'id': 'input_directory',
...
def process_events(events,
    ...
    blob_token = options_values['blob_token']
    ...
    if options_values.get('new_dated_directory', True):
        ...
        relative_path = output_directory.split('\\', 1)[-1]
        ...
        # Blob path
        blob_path_dir = f"{output_directory}\{folder_name}\JSON_LD"
        ...
        for filename in os.listdir(input_directory):
            ...
            if filename.endswith(".ttl"):
                ...
                convert_ttl_to_jsonld(ttl_file_path, blob_token, jsonld_directory, blob_path_dir)
        log("Done")
        ...
        # Blob path
        blob_path_dir = f"{output_directory}\JSON_LD"
        for filename in os.listdir(input_directory):
            # Check if the file has a .ttl extension
            if filename.endswith(".ttl"):
